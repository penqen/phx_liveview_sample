defmodule TodosAppWeb.Todos.TaskLiveTest do
  ...
  describe "Index" do
    ...
    test "lists all tasks", %{conn: conn, task: task} do
      {:ok, _index_live, html} = live(conn, ~p"/todos/tasks")
    ...
    test "saves new task", %{conn: conn} do
      {:ok, index_live, _html} = live(conn, ~p"/todos/tasks")

      assert index_live |> element("a", "New Task") |> render_click() =~
               "New Task"

      assert_patch(index_live, ~p"/todos/tasks/new")
      ...
      assert_patch(index_live, ~p"/todos/tasks")
    ...
    test "updates task in listing", %{conn: conn, task: task} do
      ...
      assert_patch(index_live, ~p"/todos/tasks/#{task}/edit")
  ...
  describe "Show" do
    ...
    test "displays task", %{conn: conn, task: task} do
      {:ok, _show_live, html} = live(conn, ~p"/todos/tasks/#{task}")
    ...
    test "updates task within modal", %{conn: conn, task: task} do
      {:ok, show_live, _html} = live(conn, ~p"/todos/tasks/#{task}")

      assert show_live |> element("a", "Edit") |> render_click() =~
               "Edit Task"

      assert_patch(show_live, ~p"/todos/tasks/#{task}/show/edit")
      ...
      assert_patch(show_live, ~p"/todos/tasks/#{task}")
